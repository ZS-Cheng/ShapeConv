...
def conv3x3(conv_cfg, in_planes, out_planes, stride=1, groups=1, dilation=1):
    """3x3 convolution with padding"""
    return build_conv_layer(conv_cfg, in_planes, out_planes, kernel_size=3, stride=stride,
                            padding=dilation, groups=groups, bias=False, dilation=dilation)


def conv1x1(conv_cfg, in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return build_conv_layer(conv_cfg, in_planes, out_planes, kernel_size=1, stride=stride, bias=False)


class BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, conv_cfg, norm_layer, act_layer, stride=1,
        ...
        # when stride != 1
        self.conv1 = conv3x3(conv_cfg, inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu1 = act_layer(planes)
        self.conv2 = conv3x3(conv_cfg, planes, planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv_cfg, norm_layer, act_layer, stride=1,
        ...
        # when stride != 1
        self.conv1 = conv1x1(conv_cfg, inplanes, width)
        self.bn1 = norm_layer(width)
        self.relu1 = act_layer(width)
        self.conv2 = ACmix(width, width, 7, 4, 3, stride=stride, dilation=dilation)
        self.bn2 = norm_layer(width)
        self.relu2 = act_layer(width)
        self.conv3 = conv1x1(conv_cfg, width, planes * self.expansion)
...
MODEL_CFGS = {
    'resnext101_32x8d': {
        ...
        'groups': 32,
        'width_per_group': 8,
        'weights_url': model_urls['resnext101_32x8d'],
    },
    'resnext50_32x4d': {
        'block': Bottleneck,
        'layer': [3, 4, 6, 3],
        'groups': 32,
        'width_per_group': 4,
        'weights_url': model_urls['resnext50_32x4d'],
    },
    'resnet152': {
        'block': Bottleneck,
        'layer': [3, 8, 36, 3],
        'weights_url': model_urls['resnet152'],
    },
    'resnet101': {
        'block': Bottleneck,
        'layer': [3, 4, 23, 3],
...
class ResNetCls(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,
                 zero_init_residual=False, input_type=None,
        ...
        if 'rgb' in input_type:
            conv1_in_channels += 3
        if 'hha' in input_type:
            conv1_in_channels += 3
        if 'depth' in input_type:
            conv1_in_channels += 1
        self.conv1 = build_conv_layer(conv_cfg, conv1_in_channels, self.inplanes, kernel_size=7, stride=2,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(conv_cfg, self.inplanes, planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, conv_cfg, norm_layer, act_layer,
                            stride, downsample, self.groups,
                            self.base_width, previous_dilation * multi_grid[0]))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, conv_cfg, norm_layer=norm_layer,
                                act_layer=act_layer,
                                groups=self.groups,
...
class ResNet(ResNetCls):
    ...
    def __init__(self, arch, replace_stride_with_dilation=None, multi_grid=None,
                 pretrain=True, input_type=None,
                 conv_cfg=None, norm_cfg=None, act_cfg=None):
        cfg = MODEL_CFGS[arch]
        block = cfg.pop('block')
        layer = cfg.pop('layer')
        weights_url = cfg.pop('weights_url')
        if input_type is None:
            input_type = ['rgb']
        super().__init__(
            block,
        ...
        if pretrain:
            ...
            state_dict = load_state_dict_from_url(weights_url)
            conv1_weight = None
            if 'rgb' in input_type or 'hha' in input_type:
                conv1_weight = state_dict['conv1.weight']
            if 'rgb' in input_type and 'hha' in input_type:
                conv1_weight = state_dict['conv1.weight'].repeat(1, 2, 1, 1)
            if 'depth' in input_type:
                r_weight = state_dict['conv1.weight'][:, 0, :, :][:, None, :, :]
                if conv1_weight is None:
                    conv1_weight = r_weight
                else:
                    conv1_weight = torch.cat([conv1_weight, r_weight], dim=1)
